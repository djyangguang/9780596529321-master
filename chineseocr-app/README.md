...
- [x]  不定长OCR训练(英文、中英文) crnn 卷积循环神经网络 \dense ocr 识别及训练 ,新增pytorch转keras模型代码(tools/pytorch_to_keras.py)
...
##类目 样本量不够 不均衡  。构造一部分数据 旋转与镜像 噪声少 直接对原图做处理。
图像像素作为特征，hog ght
KNN k个最近邻  耗内存 比较慢 改进版本 ANN 样本之间的距离。 做分类的方法。
抽取特征（简单 的是就是像素）。向量 1*N 的矩阵. 曼哈顿，余弦（空间中的方向），欧式 2个向量 的值 分别做减法.
##N折交叉验证 帮助选参数 KNN 找K个邻居 单不知道K要取几合计？无公式 
训练级分成N个 5 个 会取前4个 为正正 的训练集。第5折为交叉燕验证集。
KNN 慢准确率不高不好  ===》线性分类器 f(x,W) x =  1X3072  向量 W 权重  10X3072 得分向量。
损失函数 （代价函数）：拿到一组参数w b  后 好不好，能不能做 分类任务。
1: hinge loss henzhi 设置警戒线。 对的比不对的多出多少分 10分 才算 可以、 设计门槛 ，在标准的答案上这一类必须要比当前判断错的其他的类别要高出这个门槛。高出门槛不管没有高出 担忧。
2: softmax 互商损失
